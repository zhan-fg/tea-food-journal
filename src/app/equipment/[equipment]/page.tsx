...
import { getByEquipment, getAllContent } from "@/lib/content";
import { listAllEquipment } from "@/lib/utils";
...
export async function generateStaticParams() {
  ...
  return listAllEquipment(all).map((equip) => ({ equipment: equip }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `工具: ${equipment}`,
    description: `使用${equipment}的所有配方`,
  };
}

export default async function EquipmentPage({ params }: PageProps) {
  const { equipment } = await params;
  const recipes = getByEquipment(equipment);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <nav className="flex items-center gap-2 text-sm text-foreground/50 mb-6">
        ...
        <span className="text-foreground/80">工具</span>
        <ChevronRight className="h-3 w-3" />
        <span className="text-foreground/80">{equipment}</span>
      </nav>

      <h1 className="text-3xl font-bold text-foreground mb-2">{equipment}</h1>
      <p className="text-foreground/50 mb-8">
        使用此工具的配方 · {recipes.length} 篇
      </p>
